...
def func_2():
    ...
    if len(des1) < len(des2):
        ...
        MIN = len(des2)
    F=len(goodMatch)
    #fingerprinting similarity calculation
    global r
    if F<MIN:
       r = F / MAX
    else:
       r = MIN/MAX
    print("The matching quantity is", F)
...
l2.place(x=500, y=80)
b1 = tk.Button(window1, text="Choose queryImage",width=20, command=func_1)  
b1.place(x=650, y=80)
l3 = tk.Label(window1, text='Step2')
l3.place(x=500, y=150)
b2 = tk.Button(window1, text="Choose trainingImage", width=20, command=func_2)  
